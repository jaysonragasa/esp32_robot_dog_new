...
void setLegPWM(leg &_leg)
{
  ...
  uint8_t id0 = esp32pwmServos[l+0];
  uint8_t id1 = esp32pwmServos[l+1];
  uint8_t id2 = esp32pwmServos[l+2];
  uint16_t pulseAlpha = angleToPulse(limitServoAngle(getHALAngle(_leg.angle.alpha, _leg.hal.mid.alpha, _leg.hal.trim.alpha, _leg.hal.ratio.alpha, _leg.inverse.alpha)));
  uint16_t pulseBeta = angleToPulse(limitServoAngle(getHALAngle(_leg.angle.beta,  _leg.hal.mid.beta,  _leg.hal.trim.beta,  _leg.hal.ratio.beta,  _leg.inverse.beta )));
  uint16_t pulseGamma = angleToPulse(limitServoAngle(getHALAngle(_leg.angle.gamma, _leg.hal.mid.gamma, _leg.hal.trim.gamma, _leg.hal.ratio.gamma, _leg.inverse.gamma)));
  ESP32_ISR_Servos.setPulseWidth(id0, pulseAlpha);
  ESP32_ISR_Servos.setPulseWidth(id1, pulseBeta);
  ESP32_ISR_Servos.setPulseWidth(id2, pulseGamma);
...
void runServoCalibrate(leg &_leg)
{
  ...
  uint8_t id1 = esp32pwmServos[l+1];
  uint8_t id2 = esp32pwmServos[l+2];
  uint16_t pulse = servoMainProfile.deg90;
  ESP32_ISR_Servos.setPulseWidth(id0, pulse);
  ESP32_ISR_Servos.setPulseWidth(id1, pulse);
  ESP32_ISR_Servos.setPulseWidth(id2, pulse);
}

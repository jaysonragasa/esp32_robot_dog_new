...
planner walkPlanner(vector, body, legs[LEGLF], legs[LEGRF], legs[LEGLH],
                    legs[LEGRH]);

/* and balance it someway */
balance bodyBalance(balanceOffset, body, legs[LEGLF], legs[LEGRF], legs[LEGLH],
                    legs[LEGRH]);
...
void servicesSetup() {
  ...
  initSubscription();
  Wire.begin();
  Wire.setClock(400000); // 400 kHz Fast Mode: keeps the OLED framebuffer
                         // transfer short so it doesn't hold the I2C mutex long
                         // enough to stall the servo loop
  delay(100);
...
void servicesLoop(void *pvParameters) {
  while (1) {
    if (mainLoopReady && serviceLoopReady) {
      ...
      if (serviceCurrentTime - serviceFastPreviousTime >=
          SERVICE_FAST_LOOP_TIME) {
      ...
      if (serviceCurrentTime - servicePreviousTime >= SERVICE_LOOP_TIME) {
        ...
        serviceLoopTime =
            micros() - serviceCurrentTime; // this loop + service fast loop
...
/**
   TODO
    ...
    - make the queue of tasks by core, e.g. not just   cliFunctionFAST =
   runI2CScanFAST; but array/list with commands
